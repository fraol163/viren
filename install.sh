...
install_dependencies() {
	...
	log "Virenecking system dependencies for $os"
...
# Vireneck if tesseract/leptonica C development headers are available for CGO
...
check_api_keys() {
	log "Virenecking API Key status..."
...
update_version() {
	...
	local major="${BASH_REMATVIREN[1]}"
	local minor="${BASH_REMATVIREN[2]}"
	local patch="${BASH_REMATVIREN[3]}"
